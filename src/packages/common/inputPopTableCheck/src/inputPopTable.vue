  ...
  >
    ...
    >
      ...
      <template v-for="(item, index) in tableConfig.tableColum">
        <wg-table-column :key="index" v-bind="item" v-if="!item.notShow"></wg-table-column>
      </template>
    </wg-table>
    <!--分页区域 -->
    <wg-pagination
    ...
    <div
      ...
      v-if="
        ...
        (!tableConfig.noCancelBtn || !tableConfig.noSubmitBtn)
      "
    >
      <wg-button v-if="!tableConfig.noCancelBtn" @click="popShow = false"
        >取消</wg-button
      >
      <wg-button v-if="!tableConfig.noSubmitBtn" primary @click="submitSelect"
        >确定</wg-button
      >
    </div>
    <div
...
export default {
  ...
  watch: {
    ...
    defaultTag: {
      handler(dTag) {
        if (dTag && dTag.length !== 0) {
          const val = Array.isArray(dTag)
            ? typeof dTag[0] === "string"
              ? []
              : dTag
            : [];
          // console.log('======defaultTag===:',val);
          // console.log('======this.callBackName===',this.tableConfig.callBackName);
          this.tagsData = val.map((c) => {
            let json = {};
            json[this.tableConfig.callBackName.label] = c.label;
            json[this.tableConfig.callBackName.value] = c.value;
            json["details"] = c.details || c;
            return json;
          });
          this.selectData = val.map((c) => {
            let json = {};
            json[this.tableConfig.callBackName.label] = c.label;
            json[this.tableConfig.callBackName.value] = c.value;
            json["details"] = c.details || c;
          ...
          // 解决v-model已经清空了，但是页面展示的label还存在的问题
          this.inputVal = "";
          this.tagsData = [];
          this.selectDat = [];
        }
    ...
    inputVal(val, oldVal) {
      ...
      // 值不是空格
      if (newVal !== newOldVal) {
        console.log("==============> newVal", newVal, newOldVal);
        this.$nextTick(() => {
          // 防抖
          this.deBonce(() => {
            const len = newVal?.length || 0;
            if (len > this.minSearchLen) {
  ...
  computed: {
    ...
    triggerType() {
      return "manual";
      // if (this.minSearchLen > 0) return 'manual';
      // return this.tableConfig.immateShow ? 'click' : 'manual';
    },
  },
  mounted() {
    let self = this;
  ...
  methods: {
    initSingleSelect() {
      const { tableConfig, defaultTag } = this;
      if (defaultTag.length === 0) return;
      const { tableData, callBackName } = tableConfig;
      this.selectIndex = tableData.findIndex(
        (item) => item[callBackName.value] === defaultTag[0].value
      );
    },
    tagsInputDivClick() {
    ...
    getTagsName(data) {
      if (typeof data === "string") {
        ...
        return data[this.tableConfig.callBackName.label];
      }
    },
    deBonce(fn, timeout) {
    ...
    // 上下移动的时候改变颜色
    rowStyle(row) {
      if (this.tableConfig.isSelect || !this.openKeyboard) return;
    ...
    tagsClose(tag) {
      // 多选没有初始值,或者是选中后的删除
      if (typeof this.tagsData[0] !== "string") return this.spliceTagsData(tag);
      // 多选有初始值
      ...
      if (selectData.length === 0) {
        ...
        this.tagsData = selectData.filter((item, index) => {
          if (item[callBackName.label] === tag) {
    ...
    spliceTagsData(tag) {
      ...
      this.tagsData = this.tagsData.filter((item, index) => {
        if (callBackName?.isUseValue) {
          if (item[callBackName.value] === tag[callBackName.value]) {
    ...
    initSelect() {
      ...
      this.selectData = [];
      if (typeof this.tagsData[0] !== "string") return this.initMultiSelect();
      // 多选有初始值
      const { tableData, callBackName } = this.tableConfig;
      tableData.forEach((item) => {
        if (
          this.tagsData.includes(item[callBackName.label]) ||
          this.tagsData.includes(item[callBackName.label].toString)
        ) {
          this.$refs.multipleTable.toggleRowSelection(item);
        }
      });
    ...
    initMultiSelect() {
      ...
      if (callBackName?.isUseValue) {
        let valueData = tagsData.map((item) => item[callBackName.value]);
        tableData.forEach((item) => {
          if (valueData.includes(item[callBackName.value])) {
            this.$refs.multipleTable.toggleRowSelection(item);
          }
        });
      } else {
        let data = tagsData.map((item) => item[callBackName.value]);
        tableData.forEach((item) => {
          if (data.includes(item[callBackName.value])) {
            this.$refs.multipleTable.toggleRowSelection(item);
          }
        });
      }
    ...
    autoSelectData() {
      ...
      this.selectData = tableData.filter((item) => {
        for (let i = 0; i < tval.length; i++) {
          const index = tval[i];
          if (item[index] === this.inputVal) {
            return item;
          }
        }
      });
      if (isEmpty(this.selectData)) {
        this.callBackAction({ value: "noDataMatched", type: "noDataMatched" });
      } else {
        this.callBackAction({ type: "checked", value: [this.selectData[0]] });
      }
    },
    popvorHide() {
      if (
        ...
        this.callBackTag &&
        this.callBackTag.length !== 0
      ) {
        // event.target.value = this.callBackTag[0].label;
        // event.target.placeholder = this.placeholder;
        if (!this.enterFlag) {
          if (
            (this.manualInput &&
              isEmpty(this.inputVal) &&
              !isEmpty(this.callBackTag[0].label)) ||
            !this.manualInput
          ) {
            this.inputVal = this.callBackTag[0].label;
            this.$emit("tagInput", this.callBackTag);
          }
        }
      } else {
        this.pagerChange = false;
      }
      // 遍历表格数据，精准匹配后返回该数据
      if (
        this.manualInput &&
        !isEmpty(this.autoSelectInput) &&
        !this.tableConfig.isSelect &&
        !isEmpty(this.inputVal) &&
        isEmpty(this.selectData)
      ) {
        this.autoSelectData();
      }
      ...
      this.$emit("popHide", this.inputVal);
    ...
    handleSelectionChange(selection) {
      this.callBackTag = [];
      const t = this.getCheckData();
      this.selectData = t.concat(selection);
    },
    // 勾选checkBox
    checkBoxSelect(selection, row) {
      const { isSelect } = this.tableConfig;
      if (!isSelect) return; //单选不能触发该事件，不然会导致两次inputSearch
      const t = this.getCheckData();
      const data = t.concat(selection);
      this.tagsData = data;
    ...
    getCheckData() {
      ...
      this.tagsData.forEach((c) => {
        let index = tableData.findIndex((t) => {
          return t[callBackName.value] == c[callBackName.value];
        });
        if (index === -1) {
          t.push(c);
        }
    ...
    // 全选
    selectAll(selection, v) {
      const t = this.getCheckData();
      this.tagsData = t.concat(selection);
      this.callBackAction({ type: "checked" });
    },
    // 行选中
    handleClickTableRow(row, event, column) {
      ...
      if (!isSelect) {
        this.inputVal = row[callBackName.label];
        this.enterFlag = true;
        this.resInputFlag = true;
        this.selectData = [row];
        this.callBackAction({ type: "checked" });
    ...
    paginationChange(val) {
      ...
      console.log("==============> 分页 paramVal", paramVal);
      // 在分页的时候，不触发失焦回填v-model的回显
      (this.pagerChange = true),
        this.callBackAction({
          value: paramVal,
    ...
    selectPageMulti() {
      ...
      let data = this.tagsData.map((item) => item[callBackName.label]);
    ...
    inputFocus(event) {
      this.$nextTick(() => {
        ...
        // 单选聚焦时, 输入框里的值变灰，显示全部数据
        if (!this.tableConfig.isSelect && this.defaultTag.length !== 0) {
          this.callBackTag = [this.defaultTag[0]];
          event.target.value = "";
          event.target.placeholder =
            this.defaultTag[0]?.label || this.placeholder;
          this.inputVal = "";
          this.$emit("tagInput", []);
          return;
        }
    ...
    clear() {
      ...
      if (this.tableConfig.isSelect) {
        ...
        this.inputVal = "";
        // 重置placeholder
        this.$refs.inputRef.$refs.input.placeholder =
      ...
      if (type === "checked") {
        ...
        if (Array.isArray(value)) {
          ...
          cb = value.map((c) => {
            ...
            return {
              label: c[callBackName.label],
              value: c[callBackName.value],
              details,
            };
          });
        } else {
          ...
          cb = [
            {
              label: value[callBackName.label],
              value: value[callBackName.value],
              details,
            },
          ];
        }
        this.$emit("tagInput", cb);
      } else if (!this.tableConfig.isSelect && type === "clear") {
        this.$emit("tagInput", []);
      }
