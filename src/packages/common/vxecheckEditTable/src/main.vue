<template>
  <div :class="[singleColumnCheck?'signleTable':'','tableHeightAuto']" :style="{ width: tableWidth, height: tableHeight }">
    <vxe-table
      ref="xTable"
      border
      :stripe="stripe"
    ...
    >
      ...
      <vxe-column type="checkbox" width="28" v-if="showCheckBox" style="padding: 0 10px;"></vxe-column>
      <vxe-column type="seq" title="序号" width="60" v-if="showIndex"></vxe-column>
        <vxe-column v-if="singleColumn" show-overflow :width="singleWidth" title="全选" field="name" class-name="singleCell" >
          <template  #default="{ row,rowIndex }">
            <div v-for="(item, index) in thData" :key="index" class="single-item">
            <div v-if="item.slots&&item.slots.default !== undefined" >
              <slot :name="item.slots.default" :row="row" :rowIndex="rowIndex">
              </slot>
            </div>
            <div v-else >
              <span>{{row[item.field]}}</span>
            </div>
              <div class="divider" v-if="index<thData.length-1"></div>
            </div>
            <!--</div>-->
          </template>
        </vxe-column>
