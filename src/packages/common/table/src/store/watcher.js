...
export default Vue.extend({
  ...
  methods: {
    ...
    toggleRowSelection(row, selected, emitChange = true, checkType) {
      const changed = toggleRowStatus(this.states.selection, row, selected);
      if (changed) {
        const newSelection = (this.states.selection || []).slice();
        // 调用 API 修改选中值，不触发 select 事件
        if (emitChange) {
          if (this.states.columns[0]?.type === 'selection') {
            this.table.$emit('select', newSelection, row);
          } else {
            this.table.$emit('select', newSelection, row);
          }
        }
        if (this.states.multiOpt) {
          if (checkType === 'checkboxClick') {
          ...
          if (this.states.columns[0]?.type === 'selection') {
            ...
            this.table.$emit('selection-change', row);
          }
        }
      }
    },
