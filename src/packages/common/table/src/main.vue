...
import { addResizeListener, removeResizeListener } from '@/element/utils/resize-event';
import Mousewheel from '@/element/directives/mousewheel';
import Locale from '@/element/mixins/locale';
import Migrating from '@/element/mixins/migrating';
import { createStore, mapStates } from './store/helper';
import TableLayout from './table-layout';
import wgTableBody from './table-body';
import wgTableHeader from './table-header';
import wgTableFooter from './table-footer';
...
export default {
  ...
  methods: {
    ...
    syncPostion: throttle(20, function (e,v) {
      this.scrollPosition = 'middle';//如果启动了滚动条，那么两边fixed都需要显示浮层
  ...
  computed: {
    ...
    emptyBlockStyle() {
      ...
      let height = `calc(100% - ${this.layout.headerHeight}px`
      if (this.layout.scrollX) {
        height = `calc(100% - ${this.layout.headerHeight}px - ${this.layout.gutterWidth}px)`;
      }
      return {
        height,
        position: 'absolute',
    ...
    ...mapStates({
      multiOpt: 'multiOpt',
      onlyCheckBoxOpt: 'onlyCheckBoxOpt',
      selection: 'selection',
      columns: 'columns',
      tableData: 'data',
  ...
  mounted() {
    // 多选的复合操作
    this.store.states.multiOpt = this.multiType;
    // 只有勾选复选框才选中
    this.store.states.onlyCheckBoxOpt = this.onlyCheckBox;
    this.bindEvents();
  ...
  data() {
    ...
    return {
      ...
      // 是否拥有多级表头
      isGroup: false,
      scrollPosition: 'none',//'left', 根据业务要求，默认不显示浮层
    };
  },
};
</script>
