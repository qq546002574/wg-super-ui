import { arrayFindIndex } from '@/element/utils/util';
import { getCell, getColumnByCell, getRowIdentity } from './util';
import { getStyle, hasClass, removeClass, addClass } from '@/element/utils/dom';
...
export default {
    ...
    return (
      <table class="el-table__body" cellspacing="0" cellpadding="0" border="0">
        ...
        <tbody>
          ...
          ></el-tooltip>
          {
            this.contextMenueFlag && this.renderContextMenue()
          }
        </tbody>
      </table>
    );
  ...
  methods: {
    ...
    getRowClass(row, rowIndex) {
      ...
      // 高亮
      if (this.store.states.columns[0]?.type === 'selection') {
        if (this.table.highlightCurrentRow && this.store.states.selection.includes(row)) {
    ...
    handleContextMenu(event, row) {
      if (this.contextMenue.length !== 0) {
        // 阻止浏览器默认事件
        event.preventDefault();
        // 获取当前位置
        this.contextMenueFlag = true;
        let num2 = event.clientX;
        let num3 = document.body.clientWidth - num2;
        if (num3 < 120) {
          this.contextLeft = num2 - 150 + 'px';
        } else {
          this.contextLeft = num2 + 'px';
        }
        this.contextTop = event.clientY - 90 + 'px';
      }
    ...
    closeMenue(event) {
      event.preventDefault();
      this.contextMenueFlag = false;
    },
    // 右键菜单列点击事件
    menueClick(event, data) {
      event.stopPropagation();
      if (!data.id) return;
      this.contextMenueFlag = false;
      this.$nextTick(()=>{
        this.table.$emit('menueClick', {menuData: data, row: this.rowData, column: this.columnData})
      })
    },
    // 渲染右键菜单
    renderContextMenue() {
      const { contextMenue } = this;
      const styleObj = {
        left: this.contextLeft,
        top: this.contextTop,
      };
      return(
        <div class="menuLayer" onClick={($event) => this.closeMenue($event)}>
          <div class="wg-table-menueDiv" style={{...styleObj}}>
            <ul class="menuUl">
              {
                contextMenue.map((menue, mIndex)=>{
                  return menue.data?.map((item, index)=>{
                    return (
                      <div>
                        <li id={item.id} onClick={($event) => this.menueClick($event, item)}>{item.name}</li>
                        {
                          mIndex !== (contextMenue.length-1) && index === (menue.data.length-1) && <div class="menue-line"></div>
                        }
                      </div>)
                  })
                })
          ...
          {columns.map((column, cellIndex) => {
            ...
            return (
              <td
                ...
                rowspan={rowspan}
                colspan={colspan}
                onClick={(event) => {if(column.type === 'selection')event.stopPropagation()}}
