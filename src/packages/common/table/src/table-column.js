...
export default {
  ...
  props: {
    ...
    digit: Boolean,
    isNumber: Boolean,
    isDateTime: Boolean, // 日期时间，固定宽度，左对齐
    label: String,
    className: String,
  ...
  computed: {
    ...
    realAlign() {
      // 如果是金额，金额全部靠右
      if(this.money || this.isNumber || this.digit) return 'is-right';
      // 若是日期时间则固定居左
      if(this.isDateTime) return 'is-left';
    ...
    setColumnRenders(column) {
      ...
      if (column.type === 'expand') {
        ...
        column.renderCell = (h, data) => {
          ...
          }else if(column.isDateTime) { // 若是日期时间
            children = children ? formatDate(children) : '-';
          }else {
            children = children ? children : '-';
          }
          //如果是金额格式，统一添加 三分位添加逗号 by Mothpro
          if(column.money){
            children = formatMoney(children);
          }
          if(column.digit){
            const fkw = this.$wgFramework?.frameWindow || {};
            const t = fkw.digit ? fkw.digit : 2;
            children = formatMoney(children, '', t);
          }
