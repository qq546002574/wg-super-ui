<template>
  <transition name="el-zoom-in-top" @after-leave="$emit('dodestroy')">
    <div
      v-show="visible"
      class="wg-date-range-picker el-picker-panel el-date-range-picker el-popper"
      :class="[
        {
    ...
    >
      <div class="el-picker-panel__body-wrapper">
        ...
        <div class="el-picker-panel__body">
          <!--不需要，但是依赖其中的数据，后期需改进-->
          <div class="el-date-range-picker__time-header" style="display: none">
          ...
          >
            ...
            <date-union
              ...
              right
              :readonly="!minDate"
              @select-time="setSelectionRightTime"
            >
            </date-union>
          </div>
          <div style="clear: both"></div>
          <div class="wg-shortcuts" v-if="shortcuts">
            <div>
              <wg-button
                type="text"
                class=""
      ...
      <div class="el-picker-panel__footer" v-if="!shortcuts && showTime">
        <wg-button
          size="mini"
          class="el-picker-panel__link-btn"
          @click="handleClear"
...
import {
  ...
  extractDateFormat,
  extractTimeFormat,
} from "@/element/utils/date-util";
import Clickoutside from "@/element/utils/clickoutside";
import Locale from "@/element/mixins/locale";
import TimePicker from "./time";
import DateTable from "../basic/date-table";
import DateUnion from "./date-union.vue";
...
export default {
  ...
  computed: {
    ...
    minVisibleTime() {
      ...
      if (this.minDate) {
        const finallyVal = formatDate(this.minDate, this.timeFormat);
        if (this.showTime) {
          this.$nextTick(() => {
            if (finallyVal === "00:00:00") {
              this.$refs.dataUnion.resetTime();
            } else {
              this.$refs.dataUnion.getTime(this.minDate);
            }
          });
        }
        return finallyVal;
    ...
    maxVisibleTime() {
      ...
      if (this.maxDate || this.minDate) {
        ...
        if (this.showTime) {
          this.$nextTick(() => {
            if (finallyVal === "00:00:00") {
              this.$refs.dataRightUnion.resetTime();
            } else {
              this.$refs.dataRightUnion.getTime(this.maxDate || this.minDate);
            }
          });
        }
        return finallyVal;
  ...
  data() {
    return {
      ...
      minDate: "",
      maxDate: "",
      leftDate: new Date("2020-01-01 00:00:00"),
      rightDate: nextMonth(new Date("2020-01-01 23:59:")),
      rangeState: {
        endDate: null,
        selecting: false,
        row: null,
  ...
  watch: {
    ...
    value(newVal) {
      ...
      console.log("==========> this.rightDate", this.rightDate);
    },
  ...
  methods: {
    ...
    // 右边时间选择
    setSelectionRightTime({ type, value }) {
    ...
    handleRangePick(val, close = true, callbackClickType) {
      const defaultTime = this.defaultTime || [];
      // const minDate = modifyWithTimeString(val.minDate, defaultTime[0]);
      const minDate = modifyTime(val.minDate, 0, 0, 0);

      let maxDate = modifyWithTimeString(val.maxDate, defaultTime[1]);
      if (val.maxDate) {
        maxDate = modifyTime(val.maxDate, 23, 59, 59);
      }
      ...
      // callbackClickType === 'date-dblclick' 则表示双击确认选中日期时间，关闭弹窗；否则需要手动点确认关闭弹窗
      if (!close || (this.showTime && callbackClickType === undefined)) return;
      this.handleConfirm();
    },
    ...
    isValidValue(value) {
      let tmp = "";
      if (value[1]) {
        tmp = new Date(formatDate(value[1]) + " 00:00:00");
      }
      return (
        Array.isArray(value) &&
        value &&
        ...
        (typeof this.disabledDate === "function"
          ? !this.disabledDate(value[0]) && !this.disabledDate(tmp)
          : true)
      );
    },
  ...
  components: { TimePicker, DateTable, DateUnion },
};
</script>
