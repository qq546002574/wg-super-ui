...
export default {
  ...
  methods: {
    ...
    dblClick(event) {
      clearTimeout(this.dblTimeout); // 清除第二个单击事件
      let target = event.target;
      if (target.tagName === "SPAN") {
        target = target.parentNode.parentNode;
      }
      ...
      if (this.selectionMode === "range") {
        ...
        this.$emit("pick", newDate, "date-dblclick");
      }
    },

    handleClick(event) {
      const that = this;
      clearTimeout(that.dblTimeout); // 清除第一个单击事件
      that.dblTimeout = setTimeout(() => {
        let target = event.target;
        if (target.tagName === "SPAN") {
          target = target.parentNode.parentNode;
        }
